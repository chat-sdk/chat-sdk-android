...
public class MessagesListAdapter<MESSAGE extends IMessage>
        ...
        for (int i = 0; i < getItems().size(); i++) {
            ...
            if (messageWrapper.item instanceof Date) {
                if (i == 0) {
                    ...
                    if (getItems().get(i - 1).item instanceof Date) {
                        indicesToDelete.add(i);
                    }
                }
            }
    ...
    private boolean isPreviousSameDate(int position, Date dateToCompare) {
        if (getItems().size() <= position) return false;
        if (getItems().get(position).item instanceof IMessage) {
            Date previousPositionDate = ((MESSAGE) getItems().get(position).item).getCreatedAt();
    ...
    private boolean isPreviousSameAuthor(String id, int position) {
        ...
        if (getItems().size() <= prevPosition) return false;
        else return getItems().get(prevPosition).item instanceof IMessage
                && ((MESSAGE) getItems().get(prevPosition).item).getUser().getId().contentEquals(id);
    }
    ...
    public static abstract class BaseMessageViewHolder<MESSAGE extends IMessage>
        ...
        public void configureLinksBehavior(final TextView text) {
