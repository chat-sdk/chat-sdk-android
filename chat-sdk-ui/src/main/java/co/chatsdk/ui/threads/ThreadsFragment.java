...
public abstract class ThreadsFragment extends BaseFragment {
    ...
    protected void onThreadUpdated(List<Thread> threads, Thread thread) {
        if (adapter instanceof ThreadsListAdapter) {
            ((ThreadsListAdapter)adapter).clearData();
            ((ThreadsListAdapter)adapter).updateThreads(replaceThread(threads, thread));
        }
    }
    ...
    public void reloadData() {
        ...
        ThreadsListAdapter threadsListAdapter = (ThreadsListAdapter)adapter;
        ...
        disposableList.add(ChatSDK.events().sourceOnMain()
                ...
                .subscribe(networkEvent -> {
                    onThreadUpdated(threads, networkEvent.thread);
                }, ChatSDK::logError));
        threadsListAdapter.updateThreads(threads);
    }
