...
class EditImageActivity : BaseActivity(), OnPhotoEditorListener, View.OnClickListener,
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        var path = intent.extras?.getString(Keys.IntentKeyImagePath)
        if (path != null) {

            val bitmap = BitmapFactory.decodeFile(path)
            val rotation = ImageUtils.getCameraPhotoOrientation(path)
            val rotated = ImageUtils.rotate(bitmap, rotation.toFloat())

            mPhotoEditorView?.source?.setImageBitmap(rotated)
                    ...
                    if (created && filePath != null) {
                        ...
                        mPhotoEditor?.saveAsBitmap(saveSettings, object: OnSaveBitmap {
                            override fun onBitmapReady(saveBitmap: Bitmap?) {
                                ...
                                // Save the bitmap
                                var file = ImageUtils.saveBitmapToFile(saveBitmap)

                                intent.putExtra(Keys.IntentKeyImagePath, file.path)
