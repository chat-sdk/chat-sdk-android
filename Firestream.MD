...
FireStream has many benefits over competing solutions, here is a brief summary:

1. Open source - you can review and fork, optimise or fix the source code
2. Powered by Firebase:
  1. Low cost, excellent performance and reliability 
  2. Google isn't going out of business any time soon
  3. You have full control over YOUR users' data

### FireStream+

To support the project, we offer optional extras for FireStream. 

1. Location, Audio, Video, Image, Sticker, File messages
...
The Chat SDK is designed to be modular. FireStream takes this a stage further. In the original Chat SDK, there are essentially two layers - the UI layer and the Firebase layer. With FireStream we increase this to three layers - the UI layer, the network adapter layer and the Firebase layer. 

FireStream has a number of advantages over the   
...
```
Fly.y.sendMessageWithText("toUid", "text").subscribe((messageId, throwable) -> {
...
```
Fly.y.messageStream.subscribe(message -> {
