...
public class OpenFromPushChecker {
    ...
    public boolean checkOnCreate(Intent intent, Bundle savedInstanceState){
        if (intent.getExtras()!= null && intent.getExtras().getBoolean(BDefines.FROM_PUSH, false))
        {
            msgTimestamp = intent.getExtras().getLong(BDefines.MSG_TIMESTAMP);
            ...
            {
                intent.getExtras().remove(BDefines.FROM_PUSH);
                return true;
            }
