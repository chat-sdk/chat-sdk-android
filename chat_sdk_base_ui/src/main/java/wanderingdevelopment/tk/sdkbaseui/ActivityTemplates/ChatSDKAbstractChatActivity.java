...
import static com.braunster.chatsdk.network.BDefines.FROM_PUSH;
import static com.braunster.chatsdk.network.BDefines.LIST_POS;
...
//                if (thread.getType()==BThread.Type.Private)
    ...
    protected void initViews(){
        ...
        mSwipeRefresh.setOnRefreshListener(new SwipeRefreshLayout.OnRefreshListener() {
            ...
            public void onRefresh() {
                ...
                BNetworkManager.getThreadsInterface().loadMoreMessagesForThread(thread)
        ...
        if (messagesListAdapter == null)
            messagesListAdapter = new ChatSDKMessagesListAdapter(ChatSDKAbstractChatActivity.this, BNetworkManager.getCoreInterface().currentUserModel().getId());
    ...
    protected void onResume() {
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                ...
                BNetworkManager.getCoreInterface().getEventManager().messagesOn(thread.getEntityID(), null);
                BNetworkManager.getCoreInterface().getEventManager().threadUsersAddedOn(thread.getEntityID());
                ...
                if (!BNetworkManager.getCoreInterface().getEventManager().isListeningToThread(thread.getEntityID()))
                {
                    BNetworkManager.getCoreInterface().getEventManager().threadOn(thread.getEntityID(), null);
                }
                ...
                BNetworkManager.getCoreInterface().getEventManager().removeEventByTag(MessageListenerTAG + thread.getId());
                BNetworkManager.getCoreInterface().getEventManager().addEvent(messageEventListener);

                // Removing the last listener just to be sure we wont receive duplicates notifications.
                BNetworkManager.getCoreInterface().getEventManager().removeEventByTag(ThreadListenerTAG + thread.getId());
                BNetworkManager.getCoreInterface().getEventManager().addEvent(threadBatchedEvent);
            }
        }).start();
    ...
    protected void onStop() {
        ...
        if (thread != null && thread.getType() == BThread.Type.Public)
        {
            BNetworkManager.getThreadsInterface().removeUsersFromThread(thread, BNetworkManager.getCoreInterface().currentUserModel());
        }
    }
