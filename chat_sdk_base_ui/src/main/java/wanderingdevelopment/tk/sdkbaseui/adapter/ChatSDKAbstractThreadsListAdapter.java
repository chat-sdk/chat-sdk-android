...
public abstract class ChatSDKAbstractThreadsListAdapter<E extends ChatSDKAbstractThreadsListAdapter.ThreadListItem> extends BaseAdapter {
    ...
    public ChatSDKAbstractThreadsListAdapter(AppCompatActivity activity){
        mActivity = activity;
        initMaker();
    }

    public ChatSDKAbstractThreadsListAdapter(AppCompatActivity activity, List<E> threadItems){
        mActivity = activity;
...
//                if (DEBUG) Log.d(TAG, "BThread has more then 2 users.");
     ...
     *
     * If you want to add more data to your list you can extend the absract adapter after that you can extend the {@link ChatSDKAbstractThreadsListAdapter.ThreadListItem ThreadListItem}
...
//            if (DEBUG) logger.addSplit("fromThread" + count);
    ...
    public static class ThreadListItem{
        ...
        public ThreadListItem(BThread thread, String entityId, String name, String imageUrl, String lastMessageDate, String lastMessageText, int usersAmount, int unreadMessagesAmount, long id, Date date, boolean isPrivate) {
    ...
    protected ThreadListItemMaker<ThreadListItem> getDefaultMaker(){
        return new ThreadListItemMaker<ThreadListItem>() {
            ...
            public ThreadListItem fromBThread(BThread thread) {
                ...
                return new ThreadListItem(thread, thread.getEntityID(),
