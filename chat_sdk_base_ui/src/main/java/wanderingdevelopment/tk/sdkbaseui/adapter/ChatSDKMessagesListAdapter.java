...
import wanderingdevelopment.tk.sdkbaseui.view.ChatBubbleImageView;
        ...
        public void onClick(View v) {
            ...
            else if (StringUtils.isNotBlank(message.text))
            {
                ...
                if (!BDefines.Options.SaveImagesToDir)
                    ...
                    popupWindow = DialogUtils.getImageMessageDialog(mActivity, imageUrl, DialogUtils.ImagePopupWindow.LoadTypes.LOAD_FROM_URL, message.asBMessage());
...
//                if (DEBUG) Timber.d("dimensions[0]: %s, dimensions[1]: %s", dimensions[0], dimensions[1]);
     ...
     * The SDK does not use this feature but it can be found (commented out) in the {@link wanderingdevelopment.tk.sdkbaseui.ActivityTemplates.abstracted.ChatSDKAbstractChatActivity ChatSDKAbstractChatActivity} onDestroy() function.
     * */
