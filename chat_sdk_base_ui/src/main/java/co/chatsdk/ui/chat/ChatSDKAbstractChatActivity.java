...
import wanderingdevelopment.tk.sdkbaseui.ActivityTemplates.ChatSDKBaseActivity;
import wanderingdevelopment.tk.sdkbaseui.ActivityTemplates.ChatSDKBaseThreadActivity;
...
public abstract class ChatSDKAbstractChatActivity extends ChatSDKBaseActivity implements AbsListView.OnScrollListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (savedInstanceState != null) {
            listPos = savedInstanceState.getInt(Defines.LIST_POS, -1);
            savedInstanceState.remove(Defines.LIST_POS);
        }
...
//        }
    ...
    protected void onSaveInstanceState(Bundle outState) {
        ...
        outState.putInt(Defines.LIST_POS, listMessages.getFirstVisiblePosition());
    }
        ...
        NetworkManager.shared().a.events.source()
                ...
                .subscribe(new Consumer<NetworkEvent>() {
                    ...
                    public void accept(NetworkEvent networkEvent) throws Exception {
                        BMessage message = (BMessage) networkEvent.message;

                        if (DEBUG)
                            Timber.v("onMessageReceived, EntityID: %s", message.getEntityID());
...
//        }
    ...
    protected void showUsersDialog() {
        ChatSDKContactsFragment contactsFragment = ChatSDKContactsFragment.newThreadUsersDialogInstance(thread.getEntityID(), "CoreThread Users:", true);
    ...
    /**
     * Get the current thread from the bundle data, CoreThread could be in the getIntent or in onNewIntent.
     */
    private boolean getThread(Bundle bundle) {
        ...
        if (this.data.containsKey(THREAD_ID)) {
            ...
            if (DEBUG) Timber.e("CoreThread id is empty");
            finish();
            return false;
        }

        if (thread == null) {
            if (DEBUG) Timber.e("No CoreThread found for given ID.");
            finish();
            return false;
        }
    ...
    public void onBackPressed() {
        ...
        if (data.containsKey(Defines.FROM_PUSH)) {
            if (DEBUG) Timber.d("onBackPressed, From Push");
            data.remove(Defines.FROM_PUSH);
