...
public class ContactLinkDao extends AbstractDao<ContactLink, Long> {
    ...
    private Query<ContactLink> user_ContactLinksQuery;
    ...
    /** Internal query to resolve the "contactLinks" to-many relationship of User. */
    public List<ContactLink> _queryUser_ContactLinks(Long linkOwnerUserDaoId) {
        synchronized (this) {
            if (user_ContactLinksQuery == null) {
                QueryBuilder<ContactLink> queryBuilder = queryBuilder();
                queryBuilder.where(Properties.LinkOwnerUserDaoId.eq(null));
                user_ContactLinksQuery = queryBuilder.build();
            }
        }
        Query<ContactLink> query = user_ContactLinksQuery.forCurrentThread();
        query.setParameter(0, linkOwnerUserDaoId);
        return query.list();
    }
