...
public class BThread extends BThreadEntity  {
    ...
    public BThread(Long id, String entityID, java.util.Date creationDate, Boolean dirty, Boolean hasUnreadMessages, Boolean deleted, String name, java.util.Date LastMessageAdded, Integer type, String creatorEntityId, String imageUrl, Long creator_ID) {
        this.id = id;
    ...
    public void updateFromMap(Map<String, Object> map) {
        if (DEBUG) Log.d(TAG, "Update from map. Id: " + entityID);

        if (map == null) {
            if (DEBUG) Log.e(TAG, "Thread update from map is null, Thread ID: " + entityID);
            return;
        }
        ...
        if (map.containsKey(BDefines.Keys.BCreationDate))
        {
            ...
            else if (map.get(BDefines.Keys.BCreationDate) instanceof Double)
            {
                Double data = (Double) map.get(BDefines.Keys.BCreationDate);
                if (data != null && data > 0)
                    creationDate = new Date(/*Double.valueOf(data).longValue()*/ data.longValue());
            }
        }
        ...
        if (map.containsKey(BDefines.Keys.BType))
        {
            ...
            this.type = (int) type;
            if (DEBUG) Log.d(TAG, "Setting type to: " + this.type + " Id: " + entityID);
        }
        else
        {
        ...
        imageUrl = (String) map.get(BDefines.Keys.BImageUrl);
        creatorEntityId = (String) map.get(BDefines.Keys.BCreatorEntityId);
    }
    ...
    public Map<String, Object> asMap() {
        ...
        nestedMap.put(BDefines.Keys.BCreatorEntityId, getCreatorEntityId());

        nestedMap.put(BDefines.Keys.BImageUrl, imageUrl);
