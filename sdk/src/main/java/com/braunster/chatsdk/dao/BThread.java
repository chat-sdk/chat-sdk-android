...
public class BThread extends BThreadEntity  {
    ...
    public BThread(Long id, String entityID, java.util.Date creationDate, Boolean deleted, String name, String creatorEntityId, String type, Boolean userCreated, Boolean invitesEnabled, String description, Integer weight, Long installationId, Long creator_ID) {
        this.id = id;
    ...
    public BPath getBPath() {
        return new BPath().addPathComponent(BFirebaseDefines.Path.BThread, getEntityID());}
    ...
    public String threadImageUrl(List<BUser> users){
        ...
        if (users.size() == 1)
        {
            url = users.get(0).getPictureUrl();
        }
        if (users.size() == 2) {
            if (!users.get(0).getEntityID().equals(curUserEntity))
                url = users.get(0).getPictureThumbnail();
            else if (!users.get(1).getEntityID().equals(curUserEntity))
                url = users.get(1).getPictureThumbnail();
        }
            ...
            for (BUser user : users){
                if (!user.getEntityID().equals(curUserEntity))
                {
                    ...
                    if (StringUtils.isBlank(user.getPictureThumbnail()))
                        continue;

                    urlsAmount++;

                    url += user.getPictureThumbnail() + (urlsAmount == targetAmount ? "" : ",");
...
/*        for (BUser u : getUsers())
    ...
    public boolean isPublic(){
        return type != null && type.equals(Type.Public);
    }

    public boolean isGroup(){
        return type != null &&  type.equals(Type.Group);
    }
